...
void spectrum_pair(const char* direct, string centrality, int NoF, int NoE)
{
        ...
        if (npart > 0)
        {
          ...
          for (int i = 0; i < npart; i++)
          {
            ...
            if (abs(rap) < yCut && pt > ptMin && pt < ptMax) {
              ...
              if (id == 211 || id==-211) dNpi_ev[ptBin]++;
              if (id == 321 || id==-321) dNK_ev[ptBin]++;
