...
data_hydro_dir = "/Users/tomaspolednicek/Desktop/deuterony/spectrum/deuteron"
...
if number==2:
    spec_hadrons=pd.read_csv(data_hydro_dir+"/spectrum_lhc2760-20-30-deuteron.dat", sep="\t", header=None, names=["pt", "pion","errorpi","kaon","errorkaon","proton","errorp"])
    
if number==1:
    antispec_hadrons=pd.read_csv(data_hydro_dir+"/antispectrum_lhc2760-20-30-deuteron.dat", sep="\t", header=None, names=["pt", "pion","errorpi","kaon","errorkaon","proton","errorp"])
...
if number==1:
    ...
    axes.plot(antispec_hadrons["pt"],antispec_hadrons["pion"], color='darkcyan',linewidth=2, label="norm = 114,$\eta/s = 0.12$, $\zeta/s = 0.2$")
...
if number ==2:
    ...
    axes.plot(spec_hadrons["pt"],spec_hadrons["pion"], color='darkcyan',linewidth=2, label="norm = 114,$\eta/s = 0.12$, $\zeta/s = 0.2$")
...
axes.autoscale(tight=True)
axes.grid(True)
axes.annotate('$\mathrm{Pb+Pb\ \sqrt{s_{NN}}=2.76\ TeV}$ '+ '%s'%int(cen[:2])+' - '+'%s'%int(cen[2:4])+"%", (1.1,0.0001),fontsize=17)
