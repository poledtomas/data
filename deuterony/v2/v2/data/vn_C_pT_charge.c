...
void vn_C_pT_charge(const char* direct, int NoF, int NoE, double order, string centrality)
{
        ...
        for (int k = 0; k < eventStep; k++) {
          ...
          if (npart > 0)
          {
            ...
            for (int i = 0; i < npart; i++)
            {
              ...
              if(fabs(eta)>etaCut && pt>ptMinCut && pt<ptMaxCut &&(ele<0 || ele > 0))
              {
                RFP++; 
  ...
  ofstream fout;
  string file="vn_cumulants_pT_charge_";
  string dat= ".dat";
  string mezera = "_";
