...
void vn_EP_pT_eventbyevent(const char* direct, int NoF, int NoE, double order,string centrality){
    ...
    int ele, id;
    ofstream fout;
    string file="vn_avr_eventbyevent_";
    string dat= ".dat";
    string mezera = "_";
            ...
            for (int iev = 0; iev < NoE; iev++)
            {
                ...
                if (npart > 0)
                {
                    ...
                    for (int i = 0; i < nBins; i++)
                    {
                        ...
                        {
                             nevents[i]++;
                             //cout<< iev<<"\t" <<nevents[0]<<endl;
                             _vn_obs[i] /= _nvn[i];
                        }
                        //vn_obs[i] += _vn_obs[i];
                        sd1[i] += _vn_obs[i] * _vn_obs[i];
                        ptBin[i] = ptMinCut + (i + 0.5) * dpt;
                        fout << ptBin[i] << "\t" << _vn_obs[i] << endl;
